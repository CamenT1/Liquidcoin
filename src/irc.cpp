...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #liquidcoinTEST\r");
            Send(hSocket, "WHO #liquidcoinTEST\r");
        } else {
            // randomly join #liquidcoin00-#liquidcoin99
            int channel_number = GetRandInt(100);
            channel_number = 0; // liquidcoin: for now, just use one channel: liquidcoin00
            Send(hSocket, strprintf("JOIN #liquidcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #liquidcoin%02d\r", channel_number).c_str());
        }
