...
#else
    const char* pszModule = "liquidcoin";
#endif
    if (pex)
...
{
    // Windows: C:\Documents and Settings\username\Application Data\Liquidcoin
    // Mac: ~/Library/Application Support/Liquidcoin
    // Unix: ~/.liquidcoin
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) + "\\Liquidcoin";
#else
...
string GetConfigFile()
{
    ...
    fs::path pathConfig(GetArg("-conf", "liquidcoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override liquidcoin.conf
...
{
    namespace fs = boost::filesystem;
    fs::path pathConfig(GetArg("-pid", "liquidcoin.pid"));
    if (!pathConfig.is_complete())
        pathConfig = fs::path(GetDataDir()) / pathConfig;
    return pathConfig.string();
}
